...
#Store indenpendent and target variables
...
cat_fit = df.groupby('category')['fit'].value_counts().unstack()

cat_fit.plot(kind='barh')
...
# --------------
# Code starts here
cat_len = df.groupby('category')['length'].value_counts().unstack()
#plot_barh(df, cat_len)
